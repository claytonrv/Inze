...
class CreditCardRecordViewset(
    ...
    def get_queryset(self):
        ...
        if self.request.query_params:
            file_name_list = self.request.query_params['file__in'].split(',')
        ...
        for file in request.data.getlist('file'):
            ...
            for row in file:
                if row != b'date,category,title,amount\n':
                    recordList.append(translator.translate_csv_row_to_record(row, user, file_name))
        serializer=CreditCardRecordSerializer(data=recordList, many=True)
        print(f'Received {len(request.data.getlist("file"))} files, {len(recordList)} invoice rows in total ')
